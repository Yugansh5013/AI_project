"""
Build the analysis notebook: tables, convergence curves, Wilcoxon tests, rankings.
Run: python build_analysis_nb.py
"""
import json

cells = []

def md(source): cells.append({"cell_type": "markdown", "metadata": {}, "source": source.splitlines(True)})
def code(source): cells.append({"cell_type": "code", "metadata": {}, "source": source.splitlines(True), "execution_count": None, "outputs": []})

# ──────────────────────────────────────────────────────────────
md("""# CEC Benchmark — Full Statistical Analysis
## A-POA vs 7 Comparison Algorithms + Base POA

**Settings:** 60,000 FEs | 50 independent runs | Dim = 10

This notebook processes the raw benchmark results and produces:
1. Mean ± Std tables with per-function ranking
2. Convergence curves
3. Wilcoxon rank-sum test
4. Engineering problem comparison
""")

# ──────────────────────────────────────────────────────────────
code("""# Step 1 — Imports & Data Loading
import pandas as pd
import numpy as np
...
# ──────────────────────────────────────────────────────────────
md("""## Step 2 — Mean ± Std and Rank Tables (CEC 2014)""")

code("""# Step 2a — CEC 2014 Summary Table with Ranks
...
# ──────────────────────────────────────────────────────────────
md("""## Step 3 — Mean ± Std and Rank Tables (CEC 2017)""")

code("""# Step 3 — CEC 2017 Summary Table with Ranks
...
# ──────────────────────────────────────────────────────────────
md("""## Step 4 — Wilcoxon Rank-Sum Test

For each function, we test: **H₀: A-POA and Algorithm X have the same distribution of results.**

A p-value < 0.05 means A-POA is **statistically significantly** different from the competitor.
We also track wins (+), losses (−), and ties (≈).
""")

code("""# Step 4 — Wilcoxon Rank-Sum Test (A-POA vs each competitor)
alpha = 0.05
...
# ──────────────────────────────────────────────────────────────
md("""## Step 5 — Convergence Curves

Select representative functions from each category and plot the convergence history.
""")

code("""# Step 5 — Convergence Curves
...
# ──────────────────────────────────────────────────────────────
md("""## Step 6 — Engineering Problems Results""")

code("""# Step 6 — Engineering Problems
try:
...
# ──────────────────────────────────────────────────────────────
md("""## Step 7 — Overall Summary & Conclusions""")

code("""# Step 7 — Overall Summary
print("=" * 80)
...
print(f"Created Analysis.ipynb with {len(cells)} cells")
